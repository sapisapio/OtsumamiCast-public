...
npm run dev
```
開発時もアプリがサーバーを起動します。別途サーバーを同時起動する必要はありません。
ブラウザだけで確認するときは、アプリを終了してから npm run dev:server を使用してください。

### ポートと表示領域

アプリ上部の「このアプリのポート」を保存すると、次回起動から反映されます（既定7244）。
7244以外ではYP一覧からの接続は利用できません。接続用URL、または「IP:ポート」をリスナーに案内してください。
オーバーレイの表示領域のサイズと通知バーの位置は自動保存されます。
ElectronのuserData内のapp-settings.jsonに保存し、インストール先へは書き込みません。
...
[ISC License](LICENSE)。第三者の素材・ライブラリはそれぞれのライセンスに従います。
詳細は[公開方針](PUBLICATION.md)と[第三者の権利表記](THIRD_PARTY_NOTICES.md)を参照してください。
